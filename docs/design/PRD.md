...
- 当前主实现中，`related` 不是已落地的 frontmatter 持久字段
- 关联知识主要通过正文中的 `[[wiki link]]`、反向链接和运行时相关性规则推导
- 如后续决定把 `related` 变成显式元数据字段，应单独设计迁移与兼容策略
...
- 移动/重命名文件时，自动扫描全知识库，更新正文中引用了旧路径的 `[[wiki link]]`
- 链接图、反向链接与运行时相关性结果在后续读取时重新计算
...
- **Tab 内容**：
  ...
  - **反向链接** — 显示引用当前知识的其他条目，以及运行时计算出的关联知识
...
- **Markdown编辑器** — 集成成熟的所见即所得 Markdown 编辑器组件（当前实现采用 TipTap），提供工具栏操作，支持切换到源码模式
- **元数据面板** — 编辑器顶部或侧边以表单方式展示标题、标签（tag 输入框）、摘要等字段，用户不接触 YAML 语法
...
MemoForge 提供统一的 CLI 入口 `memoforge`。当前对外有三种接入模式，底层分为“桌面端内嵌 SSE”与“CLI fallback”两类运行形态：

**模式A：桌面协作模式（推荐）**
- 启动 Tauri 桌面应用
- 桌面应用内嵌 SSE MCP Server，对外暴露 `http://127.0.0.1:31415/mcp`
- 适用场景：本机日常使用、需要 `get_editor_state`、需要让 Agent 跟随当前桌面上下文

**AI Agent 接入配置（Claude Code / SSE）：**
```json
{
  "mcpServers": {
    "memoforge": {
...
**模式B：Headless CLI 模式（fallback）**
- 通过命令行启动独立的 MCP Server 进程
- 支持 `bound` 和 `follow` 两种模式
- 适用场景：CI/CD、服务器部署、远程主机、无桌面环境

**AI Agent 接入配置（Claude Code / stdio fallback）：**
```json
{
  "mcpServers": {
    "memoforge": {
      "command": "memoforge",
      "args": ["serve", "--mode", "bound", "--knowledge-path", "/path/to/my-knowledge"]
    }
  }
}
```
...
- 当前主架构是“桌面端内嵌 SSE + CLI fallback”混合模式
- 本机协作场景优先推荐桌面协作模式
- Headless CLI 仍保留，用于无 GUI 场景和兼容型接入

**多知识库：** 在 CLI fallback 场景下，一个 `memoforge serve` 进程服务一个知识库。如需同时访问多个知识库，启动多个进程，在 MCP 配置中注册多个 server：
...
| `move_knowledge(from, to, dry_run?)` | `from`: 原路径<br>`to`: 目标路径<br>`dry_run`: 默认 true | 移动/重命名知识文件（dry_run=true 时返回受影响的引用列表），实际执行时自动更新正文中的 `[[旧路径]]` 链接；运行时相关性结果随后重新计算 |
...
| 接口 | 参数 | 说明 |
|------|------|------|
| `get_related(path)` | `path`: 知识文件路径 | 获取关联知识（基于正文 `[[wiki link]]`、反向链接与运行时相关性规则） |
...
桌面应用和 MCP 之间的上下文同步采用**混合机制**：

- 桌面协作模式（SSE）下，`get_editor_state` 直接返回内存快照
- CLI follow 模式下，仍通过全局状态文件 `~/.memoforge/editor_state.yaml` 获取最近桌面状态

也就是说，共享状态文件目前是 CLI follow 模式的兼容桥接层，而不是桌面协作模式的主通道。

```
┌──────────────────────┐      SSE / 内存快照      ┌──────────────────────┐
│  桌面应用 (Tauri)     │ ───────────────────────► │  内嵌 MCP SSE Server   │
└──────────────────────┘                           └──────────────────────┘
            │
            │ 按需写入（CLI follow fallback）
            ▼
┌─────────────────────────────────────────────────────────┐
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `mode` | string | `"sse"`（桌面协作）/ `"follow"`（CLI 跟随）/ `"bound"`（CLI 绑定） |
...
- **语义**：通过桌面应用暴露的 SSE MCP Server 接入
- **适用场景**：本机协作场景，AI 需要实时感知用户当前上下文
- **优点**：直接读取内存状态，无需文件轮询，`get_editor_state` 实时性最好

**模式 B：CLI 绑定模式（Bound Mode）**
...
```json
{
  "mcpServers": {
    "memoforge": {
      "command": "memoforge",
      "args": ["serve", "--mode", "bound", "--knowledge-path", "/path/to/kb"]
    }
  }
}
```
...
> **当前实现状态**：
> - 桌面协作模式（SSE）已实现
> - CLI `bound` / `follow` 两种模式已实现
> - `get_editor_state` 在 SSE 模式下返回内存状态，在 CLI follow 模式下通过全局状态文件读取

**三种模式对比：**

| 特性 | 桌面协作模式(SSE) | CLI 绑定模式 | CLI 跟随模式 |
|------|-------------------|---------------|---------------|
| 配置复杂度 | 最低 | 需要指定路径 | 无需指定路径 |
| 知识库切换 | 自动跟随 | 不跟随 | 自动跟随 |
| `get_editor_state` | 内存实时 | 仅返回当前 KB 基础状态 | 依赖状态文件 |
| 适用场景 | 本机协作 | 自动化脚本/CI | 兼容型协作 |
| 状态依赖 | 桌面应用运行中 | 无 | 依赖桌面状态文件 |
...
│  Tauri + React           │    │  CLI fallback                 │
│                          │    │                              │
│  ・目录树浏览 / 编辑     │    │  ・MCP Server (stdio)         │
│  ・Markdown编辑器        │    │  ・bound / follow 模式        │
│  ・Git 状态面板          │    │  ・安全控制 + 事件日志        │
│  ・内嵌 MCP SSE Server   │    │                              │
│  ・文件监听 + 通知       │    │                              │
└──────────────┬───────────┘    └─────────┬──────┬─────────────┘
               │                          │      │
         MCP(SSE) / GUI              MCP(stdio)  HTTP(可选)
               │                          │      │
               └──────────────┬───────────┘      │
                              ▼                  ▼
                         AI Agent            Web 浏览器
...
**关键设计：桌面 SSE 优先，CLI fallback 保留**
- 桌面应用内嵌 SSE MCP Server，覆盖本机主使用场景
- `memoforge serve` 作为 fallback，覆盖 CI、服务器和无 GUI 场景
- 两条入口共享同一套 Rust 知识管理引擎，确保行为一致
- CLI follow 模式与桌面模式之间，仍可通过状态文件和事件机制协同
...
| MCP协议 | SSE + stdio | 桌面 SSE 为主，CLI stdio 为 fallback |
...
- [x] Tauri桌面应用基础框架
- [x] Markdown编辑器（阅读 / Markdown / 富文本）
- [x] 目录树浏览 + 文件管理（新建、删除、移动、重命名）
- [x] YAML frontmatter支持（title, tags, summary, summary_hash, created, updated）
- [x] MCP Server（桌面端 SSE + CLI `memoforge serve` fallback），提供核心接口
- [x] MCP readonly 模式 + 破坏性操作 dry-run 保护
- [x] 基础搜索（正则全文搜索 + 上下文预览 + 高亮）
- [x] Git集成（init, commit, pull, push, status, log），pull/push 分离，冲突时返回详细错误
- [x] 冷启动流程（新建知识库 / 打开已有目录），含基础模板知识库
- [ ] Clone Git 仓库 GUI 流程（core 基础能力已具备，桌面产品化流程仍待增强）
- [x] 文件系统监听（知识库目录变更实时刷新 GUI）

### 9.2 快速跟进（发布后两周内）

Day 1 之后立即补齐的能力，不阻塞首次发布。

- [x] 导入已有 Markdown 文件夹（扫描 .md 文件，自动生成基础 frontmatter）
- [x] 多知识库管理（切换不同知识库路径）
- [x] 标签搜索 + 组合搜索（tag:xxx + keyword）
- [x] MCP 事件日志（events.jsonl）+ 桌面应用变更通知

### 9.3 Should Have（一个月内）

- [x] 标签过滤视图
- [x] 双向链接（`[[]]` 语法）+ 反向链接面板
- [x] L0/L1/L2渐进式披露完整实现（含 summary_stale 检测、list_knowledge 分页）
- [x] Web远程访问模式（只读）
